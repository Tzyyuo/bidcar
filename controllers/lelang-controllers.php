...
if (isset($_POST['konfirmasi'])) {
    ...
    if ($status == 'dibuka') {
        ...
        header("Location: /bidcar/model/admin/data_lelang.php");
        exit;
    }
    ...
    if ($status == 'ditutup') {
        ...
        // buat update status barang jadi 'dilelang' 'sudah dilelang' atau 'belum dilelang'
        $getBarang = $koneksi->prepare("SELECT id_barang FROM tb_lelang WHERE id_lelang = ?");
        $getBarang->bind_param("i", $id_lelang);
        $getBarang->execute();
        $getResult = $getBarang->get_result();
    ...
    if (!$data) {
        echo "<script>alert('Belum ada penawaran pada lelang ini.'); window.location.href='/bidcar/model/admin/data_lelang.php';</script>";
        exit;
    }
